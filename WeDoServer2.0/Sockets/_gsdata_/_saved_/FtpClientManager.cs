...
namespace WeDoCommon.Sockets
{
    public class FtpClientManager : TcpClientManager
    {
        ...
        public FtpClientManager(IPEndPoint ipAddress, string fileName, long fileSize, string receiverId)
            : base(ipAddress.Address.ToString(), ipAddress.Port, SocUtils.GenerateFTPClientKey(SocUtils.GetFileName(fileName), fileSize, receiverId), SocConst.FTP_WAIT_TIMEOUT)
        {
//            string fileName)
  //      {
            mFullPath = fileName;
